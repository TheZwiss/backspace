// Dev-only workbench for the HiButton design pass. Nothing in the app imports
// this file; `dev-hi-button.html` is its only entry. It exists so the button
...
import { HiButton } from '../components/telemetry/answers/HiButton';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ width: 176 * scale, transform: `scale(${scale})`, transformOrigin: 'top left', height: 44 }}>
        <div className={className} style={{ display: 'flex', width: 176 }}>
          <HiButton onClick={() => undefined}>{LABEL}</HiButton>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: 'calc(100 * var(--app-vh))', background: 'rgb(var(--bg-chat))', padding: 40, display: 'flex', flexDirection: 'column', gap: 56 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <h1 style={{ fontSize: 16, fontWeight: 600, color: 'rgb(var(--text-primary))' }}>Hiii button — design workbench</h1>
      ...
      <div className="glass-modal" style={{ borderRadius: 16, padding: 24, maxWidth: 380 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <HiButton disabled={disabled} onClick={() => undefined}>{LABEL}</HiButton>
