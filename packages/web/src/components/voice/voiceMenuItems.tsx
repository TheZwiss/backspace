...
export function buildVoiceModMenuItems(targetUserId: string, channelId: string): ContextMenuItem[] {
  ...
  if (canMuteMembers) {
    items.push({
      key: 'space-mute',
      type: 'action',
      label: isSpaceMuted ? translate('runtime.selected.voiceMenuItems.spaceUnmute') : translate('runtime.selected.voiceMenuItems.spaceMute'),
  ...
  if (canDeafenMembers) {
    items.push({
      key: 'space-deafen',
      type: 'action',
      label: isSpaceDeafened ? translate('runtime.selected.voiceMenuItems.spaceUndeafen') : translate('runtime.selected.voiceMenuItems.spaceDeafen'),
      icon: React.createElement('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'currentColor', className: 'flex-shrink-0' },
  ...
  if (canDisconnectMembers) {
    ...
    items.push({
      key: 'disconnect',
      type: 'action',
      label: translate('runtime.properties.voiceMenuItems.disconnect'),
      danger: true,
      icon: React.createElement('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'currentColor', className: 'flex-shrink-0' },
  ...
  if (canMoveMembers && otherVoiceChannels.length > 0) {
    ...
    items.push({
      key: 'move-to',
      type: 'submenu',
      label: translate('runtime.properties.voiceMenuItems.moveTo'),
      icon: React.createElement('svg', { width: 14, height: 14, viewBox: '0 0 24 24', fill: 'currentColor', className: 'flex-shrink-0' },
  ...
  return (
    <div className="p-3">
      <div className="text-xs text-txt-tertiary mb-2 font-medium uppercase tracking-wider">
        <Trans i18nKey="ui.voiceMenuItems.userVolume">User Volume</Trans>
      </div>
