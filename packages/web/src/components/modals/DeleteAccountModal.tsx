...
export function DeleteAccountModal({ isOpen, onClose }: DeleteAccountModalProps) {
  ...
  const handleContinueFromWarning = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : translate('runtime.messages.DeleteAccountModal.failedToProcessSpaces'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    if (confirmUsername !== user.username) {
      setError(translate('runtime.messages.DeleteAccountModal.usernameDoesNotMatch'));
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : translate('runtime.messages.DeleteAccountModal.failedToDeleteAccount'));
      if (step === 'federation') {
  ...
  const handleDeleteAnyway = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : translate('runtime.messages.DeleteAccountModal.failedToDeleteAccount2'));
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center animate-fade-in">
      ...
      <div className="relative max-w-lg w-full mx-4 max-h-[calc(100vh-2rem)] flex flex-col bg-surface-elevated rounded-lg shadow-xl animate-slide-up overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 flex-shrink-0">
          <h2 className="text-lg font-bold text-txt-primary"><Trans i18nKey="ui.DeleteAccountModal.deleteAccount">Delete Account</Trans></h2>
          {step !== "complete" && (
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          {step === "warning" && (
            <>
              <div className="bg-accent-rose/10 border border-accent-rose/20 rounded-lg p-3.5">
                <p className="text-sm text-txt-primary font-medium mb-2"><Trans i18nKey="ui.DeleteAccountModal.thisWillPermanentlyDeleteYourAccount">This will permanently delete your account.</Trans></p>
                <ul className="text-xs text-txt-secondary space-y-1">
                  <li><Trans i18nKey="ui.DeleteAccountModal.allSpaceMembershipsWillBeRemoved">- All space memberships will be removed</Trans></li>
                  <li><Trans i18nKey="ui.DeleteAccountModal.allFriendConnectionsWillBeRemoved">- All friend connections will be removed</Trans></li>
                  <li><Trans i18nKey="ui.DeleteAccountModal.allDMMembershipsWillBeRemoved">- All DM memberships will be removed</Trans></li>
                  <li><Trans i18nKey="ui.DeleteAccountModal.yourMessagesWillRemainButBeAttributedTo">- Your messages will remain but be attributed to "Deleted User"</Trans></li>
                </ul>
              </div>
              ...
              {ownedSpaces.length > 0 && (
                <div>
                  <p className="text-sm text-txt-primary font-medium mb-2">
                    <Trans i18nKey="ui.DeleteAccountModal.youOwn">You own</Trans> {ownedSpaces.length} <Trans i18nKey="ui.DeleteAccountModal.space">space</Trans>{ownedSpaces.length > 1 ? 's' : ''}<Trans i18nKey="ui.DeleteAccountModal.handleEachBeforeContinuing">. Handle each before continuing:</Trans>
                  </p>
                    ...
                    {ownedSpaces.map(space => (
                      <div key={space.id} className="rounded-lg bg-white/[0.03] border border-white/[0.04] p-3">
                        ...
                        <div className="flex gap-2 mb-2">
                          ...
                          >
                            <Trans i18nKey="ui.DeleteAccountModal.transfer">Transfer</Trans>
                          </button>
                          <button
                          ...
                          >
                            <Trans i18nKey="ui.DeleteAccountModal.deleteSpace">Delete Space</Trans>
                          </button>
                        </div>
                        {space.action === "transfer" && (
                          <select
                          ...
                          >
                            <option value=""><Trans i18nKey="ui.DeleteAccountModal.selectNewOwner">Select new owner...</Trans></option>
                            {space.members.map(m => (
              ...
              >
                {isLoading ? translate('runtime.expressions.DeleteAccountModal.processing') : translate('runtime.expressions.DeleteAccountModal.continue')}
              </button>
            </>
          )}
          ...
          {step === "confirm" && (
            <form onSubmit={(e) => { e.preventDefault(); handleConfirmDelete(); }} className="space-y-4">
              <div className="bg-accent-rose/10 border border-accent-rose/20 rounded-lg p-3.5">
                <p className="text-sm text-txt-danger font-medium"><Trans i18nKey="ui.DeleteAccountModal.thisActionIsPermanentAndCannotBeUndone">This action is permanent and cannot be undone.</Trans></p>
              </div>

              <div>
                <label className="block text-xs text-txt-secondary mb-1.5">
                  <Trans i18nKey="ui.DeleteAccountModal.typeYourUsername">Type your username</Trans> <span className="font-mono text-txt-primary">{user.username}</span> <Trans i18nKey="ui.DeleteAccountModal.toConfirm">to confirm</Trans>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs text-txt-secondary mb-1.5"><Trans i18nKey="ui.DeleteAccountModal.password">Password</Trans></label>
                <input
                  type="password"
                  value={confirmPassword}
                  onChange={(e) => setConfirmPassword(e.target.value)}
                  className="input-danger w-full"
                  placeholder={translate("runtime.attributes.DeleteAccountModal.enterYourPassword")}
              ...
              <div className="flex gap-3">
                ...
                >
                  <Trans i18nKey="ui.DeleteAccountModal.back">Back</Trans>
                </button>
                <button
                  type="submit"
                ...
                >
                  {isLoading ? translate('runtime.expressions.DeleteAccountModal.deleting') : translate('runtime.expressions.DeleteAccountModal.deleteMyAccount')}
                </button>
              </div>
            </form>
          )}
          ...
          {step === "federation" && (
            <>
              <p className="text-sm text-txt-secondary"><Trans i18nKey="ui.DeleteAccountModal.removingYourAccountFromConnectedInstances">Removing your account from connected instances...</Trans></p>
              ...
              {federationResults.length > 0 && !deletionComplete && (
                ...
                >
                  {isLoading ? translate('runtime.expressions.DeleteAccountModal.deleting2') : translate('runtime.expressions.DeleteAccountModal.deleteAccountNow')}
                </button>
              )}
            </>
          )}
          ...
          {step === "complete" && (
            <div className="text-center py-6">
              ...
              <p className="text-lg font-medium text-txt-primary mb-1"><Trans i18nKey="ui.DeleteAccountModal.accountDeleted">Account deleted</Trans></p>
              <p className="text-sm text-txt-tertiary"><Trans i18nKey="ui.DeleteAccountModal.redirectingToLogin">Redirecting to login...</Trans></p>
            </div>
          )}
        </div>
      </div>
