...
export function CreateChannelModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError(translate('runtime.messages.CreateChannel.channelNameIsRequired'));
      return;
    }

    if (!currentSpaceId) {
      setError(translate('runtime.messages.CreateChannel.noSpaceSelected'));
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : translate('runtime.messages.CreateChannel.failedToCreateChannel'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={closeModal} title={translate("runtime.attributes.CreateChannel.createChannel")} mobileStyle="sheet">
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <label className="block text-xs font-bold text-txt-secondary uppercase mb-2">
            <Trans i18nKey="ui.CreateChannel.channelType">Channel Type</Trans>
          </label>
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-txt-tertiary">
                    {t === "text" && translate('runtime.expressions.CreateChannel.sendMessagesImagesAndFiles')}
                    {t === "voice" && translate('runtime.expressions.CreateChannel.hangOutWithVoiceAndVideo')}
                  </div>
                </div>
              </label>
            ))}
        ...
        <div className="mb-4">
          <label className="block text-xs font-bold text-txt-secondary uppercase mb-2">
            <Trans i18nKey="ui.CreateChannel.channelName">Channel Name</Trans>
          </label>
          <input
            type="text"
            value={name}
            ...
            placeholder={translate("runtime.attributes.CreateChannel.newChannel")}
            autoFocus
          />
        </div>
        ...
        {type === "text" && (
          <div className="mb-4">
            <label className="block text-xs font-bold text-txt-secondary uppercase mb-2">
              <Trans i18nKey="ui.CreateChannel.topicOptional">Topic (optional)</Trans>
            </label>
            <input
              type="text"
              value={topic}
              ...
              className="input-standard w-full"
              placeholder={translate("runtime.attributes.CreateChannel.whatSThisChannelAbout")}
            />
          </div>
        )}
        ...
        {categories.length > 0 && (
          <div className="mb-4">
            <label className="block text-xs font-bold text-txt-secondary uppercase mb-2">
              <Trans i18nKey="ui.CreateChannel.category">Category</Trans>
            </label>
            <select
              value={categoryId}
              ...
              className="input-standard w-full"
            >
              <option value=""><Trans i18nKey="ui.CreateChannel.noCategory">No Category</Trans></option>
        ...
        <div className="sticky bottom-0 z-10 pointer-events-none">
          <div className="flex justify-center pt-3 pb-1">
            <div className="glass-bubble rounded-full px-3 py-2 flex items-center gap-3 pointer-events-auto">
              ...
              >
                <Trans i18nKey="ui.CreateChannel.cancel">Cancel</Trans>
              </button>
              <button
                type="submit"
                disabled={isLoading}
              ...
              >
                {isLoading ? translate('runtime.expressions.CreateChannel.creating') : translate('runtime.expressions.CreateChannel.createChannel')}
              </button>
            </div>
          </div>
        </div>
