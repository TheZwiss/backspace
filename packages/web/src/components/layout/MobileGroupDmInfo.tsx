...
export function MobileGroupDmInfo({ params }: MobileGroupDmInfoProps) {
  ...
  if (!dmChannel) {
    return (
      <div className="flex flex-col h-full bg-surface-base">
        <MobileScreenHeader title={translate("runtime.attributes.MobileGroupDmInfo.groupInfo")} />
        <div className="flex-1 flex items-center justify-center text-txt-tertiary text-sm">
          <Trans i18nKey="ui.MobileGroupDmInfo.conversationNotFound">Conversation not found.</Trans>
        </div>
      </div>
    );
  }
  ...
  if (!dmChannel.ownerId) {
    return (
      <div className="flex flex-col h-full bg-surface-base">
        <MobileScreenHeader title={translate("runtime.attributes.MobileGroupDmInfo.info")} />
        <div className="flex-1 flex items-center justify-center text-txt-tertiary text-sm">
          <Trans i18nKey="ui.MobileGroupDmInfo.thisConversationHasNoGroupInfo">This conversation has no group info.</Trans>
        </div>
      </div>
    );
  }
  ...
  const displayName = dmChannel.name && dmChannel.name.length > 0 ? dmChannel.name : fallbackName || translate('runtime.selected.MobileGroupDmInfo.groupDM');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : translate('runtime.selected.MobileGroupDmInfo.failedToSaveSettings');
      setSaveError(msg);
  ...
  const handleConfirmLeave = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : translate('runtime.selected.MobileGroupDmInfo.failedToLeaveGroup');
      addToast(msg, 'warning', 4000);
    } finally {
      setLeaving(false);
  ...
  const handleMemberAction = async (action: DmMemberRowAction, member: User) => {
    ...
    if (action === 'remove-friend') {
      try {
        ...
        addToast(
          err instanceof Error ? err.message : translate('runtime.messages.MobileGroupDmInfo.failedToRemoveFriend'),
          'warning',
          3000,
        );
      }
  ...
  const confirmKick = async () => {
    ...
    try {
      ...
      addToast(
        translate('runtime.templates.MobileGroupDmInfo.removedFromTheGroup', { p0: pendingKick.displayName ?? parseFederatedUsername(pendingKick.username).baseName }),
        'success',
        3000,
      );
      setPendingKick(null);
      ...
      addToast(
        err instanceof Error ? err.message : translate('runtime.messages.MobileGroupDmInfo.failedToRemoveMember'),
        'warning',
        3000,
      );
    } finally {
  ...
  const confirmTransfer = async () => {
    ...
    try {
      ...
      addToast(
        translate('runtime.templates.MobileGroupDmInfo.ownershipTransferredTo', { p0: pendingTransfer.displayName ?? parseFederatedUsername(pendingTransfer.username).baseName }),
        'success',
        3000,
      );
      ...
      addToast(
        err instanceof Error ? err.message : translate('runtime.messages.MobileGroupDmInfo.failedToTransferOwnership'),
        'warning',
        3000,
      );
    } finally {
  ...
  const headerRight = editing ? (
    <button
      ...
      aria-label={translate("runtime.attributes.MobileGroupDmInfo.cancelEdit")}
    >
      <Trans i18nKey="ui.MobileGroupDmInfo.cancel">Cancel</Trans>
    </button>
  ) : null;
  ...
  return (
    <div className="flex flex-col h-full bg-surface-base">
      <MobileScreenHeader title={translate("runtime.attributes.MobileGroupDmInfo.groupInfo2")} rightActions={headerRight} />
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 px-4 pt-8 pb-6 border-b border-border-soft">
          <div className="relative">
            <button
              ...
              aria-label={editing && isOwner ? translate('runtime.expressions.MobileGroupDmInfo.changeGroupIcon') : translate('runtime.expressions.MobileGroupDmInfo.groupIcon')}
            ...
            {editing && isOwner && previewIconUrl && (
              <button
                ...
                aria-label={translate("runtime.attributes.MobileGroupDmInfo.removeGroupIcon")}
          ...
          {editing ? (
            <input
              ...
              placeholder={fallbackName || translate('runtime.expressions.MobileGroupDmInfo.groupDM')}
              maxLength={MAX_NAME_LENGTH}
              disabled={!isOwner}
              data-mobile-group-name-input
              aria-label={translate("runtime.attributes.MobileGroupDmInfo.groupName")}
            ...
            <div className="flex items-center justify-center gap-1.5 max-w-full">
              ...
              {hasFederatedMember && (
                <span data-mobile-group-globe className="inline-flex" aria-label={translate("runtime.attributes.MobileGroupDmInfo.federatedGroup")}>
                  <GroupGlobeIcon />
                </span>
              )}
            </div>
          ...
          <p className="text-xs text-txt-tertiary">
            {memberCount} {memberCount === 1 ? translate('runtime.expressions.MobileGroupDmInfo.member') : translate('runtime.expressions.MobileGroupDmInfo.members')}
          </p>
          ...
          {isOwner && !editing && (
            ...
            >
              <Trans i18nKey="ui.MobileGroupDmInfo.edit">Edit</Trans>
            </button>
          )}
        ...
        <div className="px-4 py-3 border-b border-border-soft">
          ...
          >
            ...
            <Trans i18nKey="ui.MobileGroupDmInfo.addMember">Add Member</Trans>
            {!canAddMembers && (
              <span className="text-[11px] text-txt-tertiary"><Trans i18nKey="ui.MobileGroupDmInfo.groupIsFull">— Group is full</Trans></span>
            )}
          </button>
        </div>
        ...
        <div className="p-3">
          {ownerMember && (
            <div data-mobile-group-section="owner" className="mb-4">
              <h3 className="text-[10.5px] font-bold text-txt-tertiary uppercase tracking-[0.06em] px-2 mb-1">
                <Trans i18nKey="ui.MobileGroupDmInfo.owner">OWNER</Trans>
              </h3>
          ...
          {onlineMembers.length > 0 && (
            <div data-mobile-group-section="online" className="mb-4">
              <h3 className="text-[10.5px] font-bold text-txt-tertiary uppercase tracking-[0.06em] px-2 mb-1">
                <Trans i18nKey="ui.MobileGroupDmInfo.online">ONLINE —</Trans> {onlineMembers.length}
              </h3>
          ...
          {offlineMembers.length > 0 && (
            <div data-mobile-group-section="offline" className="opacity-60">
              <h3 className="text-[10.5px] font-bold text-txt-tertiary uppercase tracking-[0.06em] px-2 mb-1">
                <Trans i18nKey="ui.MobileGroupDmInfo.offline">OFFLINE —</Trans> {offlineMembers.length}
              </h3>
        ...
        <div className="px-4 py-4 border-t border-border-soft">
          ...
          >
            {leaving ? translate('runtime.expressions.MobileGroupDmInfo.leaving') : translate('runtime.expressions.MobileGroupDmInfo.leaveGroup')}
          </button>
        </div>
      </div>
      ...
      {editing && (
        ...
        >
          ...
          >
            <Trans i18nKey="ui.MobileGroupDmInfo.cancel2">Cancel</Trans>
          </button>
          <button
            type="button"
            onClick={handleSave}
          ...
          >
            {saving ? translate('runtime.expressions.MobileGroupDmInfo.saving') : translate('runtime.expressions.MobileGroupDmInfo.save')}
          </button>
        </div>
      )}
      ...
      <ImageCropModal
        ...
        title={translate("runtime.attributes.MobileGroupDmInfo.cropGroupIcon")}
        cropShape="round"
        aspectRatio={1}
      ...
      <ConfirmDialog
        ...
        title={translate("runtime.attributes.MobileGroupDmInfo.leaveGroup")}
        description={translate('runtime.templates.MobileGroupDmInfo.leaveYouWillStopReceivingMessagesFromThis', { p0: displayName })}
        confirmLabel={translate("runtime.attributes.MobileGroupDmInfo.leave")}
        variant="danger"
        loading={leaving}
      />
      ...
      <ConfirmDialog
        ...
        title={translate("runtime.attributes.MobileGroupDmInfo.removeFromGroup")}
        description={
          pendingKick
            ? translate('runtime.templates.MobileGroupDmInfo.removeFromThisGroupTheyWonTBe', { p0: pendingKick.displayName ?? parseFederatedUsername(pendingKick.username).baseName })
            : ''
        }
        confirmLabel={translate("runtime.attributes.MobileGroupDmInfo.remove")}
        variant="danger"
      ...
      <ConfirmDialog
        ...
        title={translate("runtime.attributes.MobileGroupDmInfo.transferOwnership")}
        description={
          pendingTransfer
            ? translate('runtime.templates.MobileGroupDmInfo.transferOwnershipToYouLlLoseOwnerPrivileges', { p0: pendingTransfer.displayName ?? parseFederatedUsername(pendingTransfer.username).baseName })
            : ''
        }
        confirmLabel={translate("runtime.attributes.MobileGroupDmInfo.transfer")}
        variant="warning"
        loading={submittingMemberAction}
      />
    </div>
