...
export function MobileSpacesScreen() {
  ...
  const handleCreateFolder = async (spaceId: string) => {
    ...
    addToast(translate('runtime.messages.MobileSpacesScreen.folderCreated'), 'success', 3000);
  };
  ...
  const handleMoveToFolder = async (spaceId: string, folderId: string) => {
    ...
    await updateSpaceLayout(newLayout, folderData);
    addToast(translate('runtime.messages.MobileSpacesScreen.movedToFolder'), 'success', 3000);
  };
  ...
  const handleRemoveFromFolder = async (spaceId: string) => {
    ...
    await updateSpaceLayout(newLayout, folderData);
    addToast(translate('runtime.messages.MobileSpacesScreen.removedFromFolder'), 'success', 3000);
  };
  ...
  const handleUngroup = async (folderId: string) => {
    ...
    addToast(translate('runtime.messages.MobileSpacesScreen.folderUngrouped'), 'success', 3000);
  };
  ...
  const handleSpaceContextMenu = (e: React.MouseEvent, spaceId: string) => {
    ...
    const items: ContextMenuItem[] = [
      {
        key: 'invite',
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.invitePeople'),
        ...
        onClick: async () => {
          try {
            ...
            addToast(translate('runtime.messages.MobileSpacesScreen.inviteLinkCopiedToClipboard'), 'success', 3000);
          } catch {
            addToast(translate('runtime.messages.MobileSpacesScreen.failedToGenerateInvite'), 'warning', 3000);
          }
        },
      },
      {
        key: 'create-folder',
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.createFolder'),
        hidden: inFolder,
    ...
    if (!inFolder && availableFolders.length > 0) {
      items.push({
        key: 'move-to-folder',
        type: 'submenu',
        label: translate('runtime.properties.MobileSpacesScreen.moveToFolder'),
        ...
        children: availableFolders.map(f => ({
          ...
          label: f.name || translate('runtime.selected.MobileSpacesScreen.unnamedFolder'),
    ...
    if (inFolder) {
      items.push({
        ...
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.removeFromFolder'),
    ...
    items.push({
      key: 'transfer',
      type: 'action',
      label: translate('runtime.properties.MobileSpacesScreen.transferOwnership'),
      hidden: !isOwner,
    ...
    items.push({
      key: 'leave',
      type: 'action',
      label: translate('runtime.properties.MobileSpacesScreen.leaveSpace'),
      hidden: isOwner,
      danger: true,
    ...
    const items: ContextMenuItem[] = [
      {
        ...
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.channelSettings'),
      ...
      { key: 'ch-sep', type: 'separator' },
      {
        key: 'delete-channel',
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.deleteChannel'),
        danger: true,
    ...
    openContextMenu({ x: e.clientX, y: e.clientY }, [
      {
        ...
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.categorySettings'),
        icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>,
      ...
      { key: 'cat-sep', type: 'separator' },
      {
        key: 'delete-category',
        type: 'action',
        label: translate('runtime.properties.MobileSpacesScreen.deleteCategory'),
        danger: true,
        icon: <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>,
  ...
  const handleVoiceUserContextMenu = useCallback(
    (e: React.MouseEvent, userId: string, channelId: string) => {
      ...
      items.push({
        key: 'mute-user',
        type: 'checkbox',
        label: translate('runtime.properties.MobileSpacesScreen.muteUser'),
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-surface-channel">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {showChannelSkeleton ? (
            <div className="px-1 pt-1" role="status" aria-label={translate("runtime.attributes.MobileSpacesScreen.loadingChannels")}>
            ...
            <>
              ...
              {isSpaceSettledEmpty && (
                <div className="flex flex-col items-center justify-center h-32 opacity-80">
                  ...
                  <p className="text-txt-tertiary text-sm"><Trans i18nKey="ui.MobileSpacesScreen.noChannelsYet">No channels yet.</Trans></p>
                </div>
              )}
            </>
          )}
          ...
          >
            ...
            <div className="py-2">
              ...
              >
                ...
                <span className="text-sm text-txt-primary"><Trans i18nKey="ui.MobileSpacesScreen.createSpace">Create Space</Trans></span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-sm text-txt-primary"><Trans i18nKey="ui.MobileSpacesScreen.joinSpace">Join Space</Trans></span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-sm text-txt-primary"><Trans i18nKey="ui.MobileSpacesScreen.exploreSpaces">Explore Spaces</Trans></span>
              </button>
            </div>
          </div>
        </>
      ...
      {leaveConfirmSpaceId && (
        <ConfirmDialog
          isOpen={true}
          title={translate("runtime.attributes.MobileSpacesScreen.leaveSpace")}
          description={translate("runtime.attributes.MobileSpacesScreen.areYouSureYouWantToLeaveThis")}
          confirmLabel={translate("runtime.attributes.MobileSpacesScreen.leave")}
          variant="danger"
          onConfirm={async () => {
            try {
              ...
              addToast(translate("runtime.messages.MobileSpacesScreen.leftSpace"), "success", 3000);
            } catch {
              addToast(translate("runtime.messages.MobileSpacesScreen.failedToLeaveSpace"), "warning", 3000);
            }
      ...
      {deleteChannelId && (
        <ConfirmDialog
          isOpen={true}
          title={translate("runtime.attributes.MobileSpacesScreen.deleteChannel")}
          description={translate("runtime.attributes.MobileSpacesScreen.areYouSureThisWillPermanentlyDeleteThe")}
          confirmLabel={translate("runtime.attributes.MobileSpacesScreen.delete")}
          variant="danger"
          onConfirm={async () => {
            try {
              await useSpaceStore.getState().deleteChannel(deleteChannelId);
              addToast(translate("runtime.messages.MobileSpacesScreen.channelDeleted"), "success", 3000);
            } catch {
              addToast(translate("runtime.messages.MobileSpacesScreen.failedToDeleteChannel"), "warning", 3000);
            }
      ...
      {deleteCategoryId && (
        <ConfirmDialog
          isOpen={true}
          title={translate("runtime.attributes.MobileSpacesScreen.deleteCategory")}
          description={translate("runtime.attributes.MobileSpacesScreen.areYouSureChannelsInThisCategoryWill")}
          confirmLabel={translate("runtime.attributes.MobileSpacesScreen.delete2")}
          variant="danger"
          onConfirm={async () => {
            try {
              await useSpaceStore.getState().deleteCategory(deleteCategoryId);
              addToast(translate("runtime.messages.MobileSpacesScreen.categoryDeleted"), "success", 3000);
            } catch {
              addToast(translate("runtime.messages.MobileSpacesScreen.failedToDeleteCategory"), "warning", 3000);
            }
