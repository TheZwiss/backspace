...
export function MainContent() {
  ...
  if (showDms || isExplorePage || !currentSpaceId) {
    ...
    const dmName = isGroupDm && dmChannel
      ...
      : (firstOther?.displayName ?? (firstBaseName || translate('runtime.selected.MainContent.directMessage')));
    ...
    const dmInputPlaceholder = isGroupDm && dmChannel
      ? translate('runtime.templates.MainContent.message', { p0: formatDmInputLabel(dmChannel, authUser) })
      : dmChannel
        ? translate('runtime.templates.MainContent.message2', { p0: dmName })
        : undefined;
        ...
        >
          <div className={`h-14 px-5 flex items-center justify-between border-b border-border-hard flex-shrink-0 bg-surface-base transition-opacity duration-300 ${voiceFullscreen ? 'opacity-0 hover:opacity-100' : ''}`}>
            <div className="flex items-center gap-[10px]">
              ...
              {connectionError ? (
                <span className="text-xs text-txt-danger font-medium ml-2"><Trans i18nKey="ui.MainContent.connectionFailed">Connection Failed</Trans></span>
              ) : isLiveKitConnected ? (
                <>
                  <span className="text-xs text-status-online font-medium ml-2"><Trans i18nKey="ui.MainContent.connected">Connected</Trans></span>
                  <span className="text-xs text-txt-tertiary ml-1">{participants.length} <Trans i18nKey="ui.MainContent.inCall">in call</Trans></span>
                </>
              ) : (
                <span className="text-xs text-status-idle font-medium ml-2"><Trans i18nKey="ui.MainContent.connecting">Connecting...</Trans></span>
              )}
            </div>
        ...
        {isCallingThisDm && (
          <div className="bg-status-online/10 border-b border-status-online/20 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-status-online text-sm font-medium"><Trans i18nKey="ui.MainContent.calling">Calling</Trans> {dmName}...</span>
            </div>
            <button
              onClick={handleCancelCall}
              className="px-3 py-1 bg-accent-rose hover:bg-accent-rose/80 text-white text-xs font-medium rounded transition-colors"
            >
              <Trans i18nKey="ui.MainContent.cancel">Cancel</Trans>
            </button>
          </div>
        )}
        <div className="h-14 px-5 flex items-center justify-between border-b border-border-hard flex-shrink-0 z-10 bg-surface-chat">
          <div className="flex items-center gap-[10px] min-w-0">
            {isGroupDm ? (
              <div
                ...
                aria-label={translate("runtime.attributes.MainContent.openGroupSettings")}
              >
            ...
            {isGroupDm && (
              ...
              >
                ({dmChannel?.members.length} <Trans i18nKey="ui.MainContent.members">members)</Trans>
              </span>
            )}
            {isGroupDm && (
              <button
                ...
                title={translate("runtime.attributes.MainContent.groupSettings")}
              >
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button
              ...
              title={translate("runtime.attributes.MainContent.startVoiceCall")}
            >
            ...
            <button
              ...
              disabled={!!outgoingCall || !!activeDmCall}
              className="w-8 h-8 flex items-center justify-center text-txt-tertiary hover:text-txt-primary transition-colors rounded-[6px] hover:bg-interactive-hover disabled:opacity-50 disabled:cursor-not-allowed"
              title={translate("runtime.attributes.MainContent.startVideoCall")}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            <button
              ...
              title={translate("runtime.attributes.MainContent.addFriendsToDM")}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            <button
              ...
              title={translate("runtime.attributes.MainContent.search")}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
    ...
    return (
      <div className="flex-1 flex flex-col bg-surface-chat relative">
        <div className="h-14 px-5 flex items-center justify-between border-b border-border-hard">
          <span className="text-txt-tertiary"><Trans i18nKey="ui.MainContent.selectAChannel">Select a channel</Trans></span>
          <div className="flex items-center gap-1 flex-shrink-0">
            <TransferIndicator />
            <MemberListToggleButton />
          </div>
        ...
        <div className="flex-1 flex items-center justify-center text-txt-tertiary">
          <p><Trans i18nKey="ui.MainContent.selectATextOrVoiceChannelToGet">Select a text or voice channel to get started</Trans></p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex-1 flex flex-col bg-surface-base">
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-8 relative">
            ...
            <div className="text-center relative z-10">
              ...
              <p className="text-txt-tertiary text-[15px]"><Trans i18nKey="ui.MainContent.noOneIsCurrentlyInThisVoiceChannel">No one is currently in this voice channel.</Trans></p>
            </div>
            <button
            ...
            >
              <Trans i18nKey="ui.MainContent.joinVoice">Join Voice</Trans>
            </button>
          </div>
        </div>
      );
      ...
      >
        <div className={`h-14 px-5 flex items-center justify-between border-b border-border-hard flex-shrink-0 bg-surface-base transition-opacity duration-300 ${voiceFullscreen ? 'opacity-0 hover:opacity-100' : ''}`}>
          <div className="flex items-center gap-[10px]">
            ...
            <span className="font-bold text-[15px] tracking-[-0.02em] text-txt-primary">{channel.name}</span>
            {connectionError ? (
              <span className="text-xs text-txt-danger font-medium ml-2"><Trans i18nKey="ui.MainContent.connectionFailed2">Connection Failed</Trans></span>
            ) : isLiveKitConnected ? (
              <>
                <span className="text-xs text-status-online font-medium ml-2"><Trans i18nKey="ui.MainContent.connected2">Connected</Trans></span>
                <span className="text-xs text-txt-tertiary ml-1">{participants.length} <Trans i18nKey="ui.MainContent.connected3">connected</Trans></span>
              </>
            ) : (
              <span className="text-xs text-status-idle font-medium ml-2"><Trans i18nKey="ui.MainContent.connecting2">Connecting...</Trans></span>
            )}
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            <TransferIndicator />
  ...
  return (
    <div className="flex-1 flex flex-col bg-surface-chat min-w-0 relative">
      <div className="h-14 px-5 flex items-center justify-between border-b border-border-hard flex-shrink-0 z-10 bg-surface-chat">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button className="w-8 h-8 flex items-center justify-center text-txt-tertiary hover:text-txt-primary transition-colors rounded-[6px] hover:bg-interactive-hover" title={translate("runtime.attributes.MainContent.notificationSettings")}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
          ...
          <button
            ...
            onClick={() => setSearchOpen(!searchOpen)}
            className={`w-8 h-8 flex items-center justify-center transition-colors rounded-[6px] ${searchOpen ? 'text-txt-primary bg-interactive-active' : 'text-txt-tertiary hover:text-txt-primary hover:bg-interactive-hover'}`}
            title={translate("runtime.attributes.MainContent.search2")}
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              <path d="M21.707 20.293l-5.395-5.395A7.457 7.457 0 0018 10.5 7.5 7.5 0 1010.5 18c1.575 0 3.027-.486 4.228-1.31l5.476 5.476a.997.997 0 001.414 0l.089-.089a1 1 0 000-1.414l.001-.37zM10.5 16a5.5 5.5 0 110-11 5.5 5.5 0 010 11z" />
            </svg>
