...
export function buildMessageMenuItems(params: MessageMenuParams): ContextMenuItem[] {
  ...
  if (imageUrl) {
    items.push({
      key: 'save-image',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.saveImage'),
      icon: (
    ...
    items.push({
      key: 'copy-image',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.copyImage'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    ...
    if (!sourceUrl) {
      items.push({
        key: 'open-original',
        type: 'action',
        label: translate('runtime.properties.messageMenuItems.openOriginal'),
        icon: (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  ...
  if (videoUrl && videoFilename) {
    items.push({
      key: 'save-video',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.saveVideo'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" />
        </svg>
  ...
  if (audioUrl && audioFilename) {
    items.push({
      key: 'save-audio',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.saveAudio'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" />
        </svg>
  ...
  if (sourceUrl) {
    items.push({
      key: 'copy-link',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.copyLink'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      onClick: () => {
        navigator.clipboard.writeText(sourceUrl).then(() => {
          useUIStore.getState().addToast(translate('runtime.messages.messageMenuItems.copiedLink'), 'success', 3000);
        }).catch(() => {
          useUIStore.getState().addToast(translate('runtime.messages.messageMenuItems.failedToCopyLink'), 'warning', 3000);
        });
      },
    });
    items.push({
      key: 'open-link',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.openLink'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19 19H5V5h7V3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z" />
        </svg>
      ...
      render: () => (
        <div className="flex items-center gap-1 px-2 py-1.5">
          ...
          <button
            ...
            title={translate("runtime.attributes.messageMenuItems.addReaction")}
          >
  ...
  if (canReply) {
    items.push({
      key: 'reply',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.reply'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  ...
  items.push({
    key: 'copy-selected',
    type: 'action',
    label: translate('runtime.properties.messageMenuItems.copySelectedText'),
  ...
  items.push({
    key: 'copy-text',
    type: 'action',
    label: translate('runtime.properties.messageMenuItems.copyText'),
  ...
  items.push({
    key: 'mark-unread',
    type: 'action',
    label: translate('runtime.properties.messageMenuItems.markUnread'),
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  ...
  if (isAuthor) {
    items.push({
      key: 'edit',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.editMessage'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
  ...
  if (canDelete) {
    items.push({
      key: 'delete',
      type: 'action',
      label: translate('runtime.properties.messageMenuItems.deleteMessage'),
      icon: (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
