...
function WelcomeHeader({ channelId }: { channelId: string }) {
  ...
  if (isDm) {
    ...
    if (isGroupDm) {
      ...
      return (
        <WelcomeHero kind="group" figure={<AvatarStack members={otherMembers} size={80} border="chat" iconUrl={dm.icon} />} title={groupName}>
    ...
    return (
      <WelcomeHero kind="dm" figure={<ProfileAvatar src={otherUser?.avatar} name={displayName} size={80} user={otherUser ?? undefined} />} title={displayName}>
        <p className="text-txt-secondary text-[14px] mt-1">
          <Trans
            t={t}
  ...
  return (
    <WelcomeHero kind="channel" figure={hashDisc} title={t('chat:list.welcome.channel.title')}>
