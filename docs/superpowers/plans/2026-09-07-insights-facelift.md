...
- **Sweep prose on three axes, not one.** Fifteen clauses on this plan have gone false, and they fail in three distinct ways. (1) **Location**: a phrase pointing at something no longer beside it, such as "the country ranking below" once the ranking is elsewhere. (2) **Subject**: a paragraph goes from describing two series to three, and a sentence true of two is false of the third, such as history "reconstructed from per-star and per-fork timestamps" once watchers join, which have no such record. (3) **Surroundings**: a clause that never moved at all and went false because the page changed around it, such as a comment naming a 1034-pixel canvas beside a chart now 487px wide, or a string saying "a ranked section declined to plot" once ranked sections are cards.

  The third is the dangerous one, because **it is invisible in the diff**: those lines are not in the diff at all. Reading the diff, however carefully, cannot find them. The check that does is a grep of the whole file for the nouns the task retired, the slot and panel ids it deleted, and any measurement whose value it changed. Task 8's review found five of these and its implementer then found four more by deriving that grep, so a sweep of the first two axes plus a careful diff read is demonstrably not enough. **Report what you cleared and why, not only what you changed**, and do not describe a sweep as complete unless it covered all three.
...
- **Pin `releaseTrend`'s window edges, which no mode covers today.** Add a fixture mode `release-edge`: the same shape as `long`, with one release dated exactly on `priorStart` (`E - 59 * D`), one inside the prior window, one dated exactly on `recentStart` (`E - 29 * D`), and three inside the recent window. Assert the chip reads `+2 · 30d` and the `chipTitle` says `4 releases shipped in the 30 days to` and `against 2 in the 30 days before them`, so each boundary release is counted in exactly one window, the right one. **Both edges, not just the outer one.** `priorStart` alone leaves the seam between the two windows untested, and the seam is where a swap or a double count actually happens: in `long` and in this mode's first draft the nearest release to it was fifteen days away on either side, so an off-by-one there read identically under both. **Four against two, not one against one.** A trend of `0` is the value a swapped window, an off-by-one at either edge and a double count all produce, and it is the exact failure that got this plan's first fixture rewritten. **The chip alone is not the assertion.** A dropped outer boundary (4 against 1) and a double-counted one (5 against 2) both read `+3`; the `chipTitle` states both totals and separates them, which is why the assertion names it. **Falsify by moving a boundary release one day earlier, not later**, once for each edge. Moving the `priorStart` release one day later keeps it inside the prior window and changes nothing; one day earlier leaves that window and reads `+3`, `4 ... against 1`. Moving the `recentStart` release one day earlier carries it across the seam into the prior window and reads `0`, `3 ... against 3`. (Two revisions: the first draft of this paragraph asked for two against two and for the wrong falsification direction, and Task 9's implementer caught both rather than building to them; its second asked for the outer edge only while the mode's name and the docs claimed both, which Task 9's review caught.)
...
- At 1440px the lead row is four columns; from 561px to 999px it is two columns and two rows deep, with the values at the heading size; at 560px and below it is one column and four rows deep, which is Task 4's `max-width: 560px` rule. Check all three. **Do not check that the row leaves the sticky range control above the fold: it does not, at any width, and that is the accepted cost of the row sitting above the control.** Record the control's document offset instead; the Final verification carries the measured table.
...
- [ ] The page at 1440px, 1000px, 984px, 910px, 900px, 760px, 560px and 380px: the body never scrolls sideways, the hero cards scroll inside their own containers below about 570px, and no compact card scrolls at any width (240px minimum against a 320px viewport's 256px host). **A hero plot is taller than every compact plot in its group, and only a zero-based hero carries an area fill.** That is the invariant the harness proves and the one that holds; see the line below for the one this check used to make and could not keep.

  The lead row is four columns at and above 1000px, two from 561px to 999px, and one at 560px and below. The series band drops to two columns just under 984px and the detail band to one just under 910px; both are intrinsic thresholds of `minmax(300px, 1fr)` and `minmax(420px, 1fr)` against the wrap's inner width rather than breakpoints anything declares, so the exact pixel is derived and the named widths are the last at which the wider count still holds.
- [ ] **Two things this section used to assert that are not true, recorded rather than chased.** Neither is a defect introduced by this plan and neither is fixable inside it.

  **The hero is not taller than every card under it, in two of the four groups.** Measured in Chrome at 1440px on the `long` fixture: Adoption's hero canvas is 280px and so are both of its telemetry cards, all three 1072px wide, because Track F draws them in a full-width `chart-stack` at the hero profile. And by card height Reach's hero is the shortest card in its group (424px against 496, 606 and 627), because the cards under it carry ranked lists and standing prose that the hero does not. The hierarchy is carried by plot height, panel colour and the group's lead figure, and it holds against every *compact* card; it does not hold against the detail cards or against Track F's stack. Fixing either means changing cards this plan's "Not in this plan" forbids it to touch.

  **Adoption's half of this was fixed after the plan closed**, in the round that answers the whole-branch review's I3: the two fleet cards mount at the `compact` profile and sit in the detail band's own tracks, measured at 487x150 against the hero's 1034x280 at 1440px, and the `chart-stack` they sat in has gone with the CSS rule that had to span it across every track. Reach's half stands exactly as recorded above, for the reason given: its hero is the shortest card in its group because the cards under it carry ranked lists and standing prose.

  **The lead row leaves the sticky range control below the first screen, at every width.** The control is `position: sticky; top: 60px`, so it is never lost, only not in the first screenful; it pins itself on the first scroll, which a reader must make to reach any chart it governs, and the default range is `all`. The row is above the control by spec section 5.1 and Task 9 Step 3, for a reason that stands: the four figures are measured over a fixed 30 days and putting them under a control that does not govern them invites the reading that it does. The cost, measured on the `long` fixture, in document coordinates:

  | viewport width | lead row | control bottom | with the row hidden | cost |
  |---|---|---|---|---|
  | 1440px | 4 columns, 1 row | 910px | 688px | 222px |
  | 1000px | 4 columns, 1 row | 907px | 685px | 222px |
  | 999px | 2 columns, 2 rows | 972px | 685px | 287px |
  | 900px | 2 columns, 2 rows | 966px | 679px | 287px |
  | 760px | 2 columns, 2 rows | 959px | 672px | 287px |
  | 570px | 2 columns, 2 rows | 998px | 725px | 273px |
  | 560px | 1 column, 4 rows | 1302px | 725px | 577px |
  | 380px | 1 column, 4 rows | 1527px | 950px | 577px |
  | 320px | 1 column, 4 rows | 1646px | 1069px | 577px |

  So a 1440x900 laptop has it off the first screen, and every phone has it two to three screenfuls down. No trim recovers this while the row stays above the control: zeroing the row's 30px top margin recovers a tenth of the narrowest case. The one worth raising with the owner is the 560px and below case, where four full-size figures stacked cost 577px and a two-column rule at phone widths would halve it. That rule is Task 3's, not Task 9's, and changing it is a design decision on a page the owner has not yet seen.
...
- [ ] `git log --oneline origin/main..HEAD` shows every commit on the branch with no attribution trailer of any kind and no session link. **Use `origin/main`, not `main`:** the local ref goes stale during a long run and the range then reads against a commit `main` has long since passed. **Do not expect nine commits.** The plan has nine tasks, not nine commits: each task carries its own fix rounds, the controller merges each with `--no-ff`, and the plan and spec are amended on the branch as reviews land. The real count is in the dozens and that is correct. What must hold is the file scope, the commit message style and the absence of trailers, not an arithmetic that was never true.
