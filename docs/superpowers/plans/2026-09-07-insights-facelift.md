...
- **Pin `releaseTrend`'s window edges, which no mode covers today.** Add a fixture mode `release-edge`: the same shape as `long`, with one release dated exactly on `priorStart` (`E - 59 * D`), one inside the prior window, and three inside the recent window. Assert the chip reads `+1 · 30d` and the `chipTitle` says `3 releases shipped in the 30 days to` and `against 2 in the 30 days before them`, so the boundary release is counted in the prior window and not in the recent one and not in both. **Three against two, not two against two.** Two against two is a trend of `0`, which is the value a swapped window, an off-by-one at either edge and a double count all produce, and it is the exact failure that got this plan's first fixture rewritten. **Falsify by moving the boundary release one day earlier, not later.** One day later is still inside the prior window and changes nothing; one day earlier leaves the prior window entirely and moves the chip to `+2`. (This paragraph was wrong in both respects when first written, and Task 9's implementer caught it rather than building to it.)
