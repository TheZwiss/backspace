...
All four assets are produced by `scripts/gen-icons.mjs` from `assets/brand/{mark.svg, mark-mono-dark.svg}` — see `scripts/gen-icons.README.md` for the full output matrix and regeneration workflow. These are the only app-icon-badge-adjacent outputs still sourced from the bare mark rather than the `app-icon.svg` badge composition; a future pass may retune them for the tray's small size specifically.
...
All desktop and web brand assets are generated by `scripts/gen-icons.mjs` from sources in `assets/brand/`. Run via `pnpm gen-icons` after artwork changes; commit the diff. The generator uses `sharp` (SVG → PNG via librsvg, including the app icon's `<filter>` drop/inner shadow and its soft-light stroke overlay), `png-to-ico` (multi-size `.ico`), and `png2icons` (`.icns`). Output is byte-stable for a given lockfile.

**Sources:**
- `app-icon.svg` — the full badge composition (squircle, gradient fill, drop shadow, inner shadow) at a 256 viewBox. Every app-icon output above 32 px renders straight from this vector, at target size, with no raster intermediate or post-render masking — the squircle and its rounding are baked into the SVG itself.
- `app-icon-small.svg` — the same badge composition with a bolder, simplified mark. Drives app-icon outputs at 16 and 32 px, where the standard mark's inset strokes and soft-light overlay read as noise.
- `mark.svg` — the bare gradient glyph. Drives the Win/Linux tray icons, the PWA maskable icon's inner mark (composited over a full-bleed copy of the badge's navy gradient, mark at 60% of canvas height), and `packages/web/public/icons/logo-mark.svg` (byte copy).
- `mark-small.svg` — a bolder small-size variant of the glyph. Drives the web favicons (16/32, transparent, glyph fills the box).
- `mark-mono-dark.svg` — solid-black glyph silhouette. Drives the macOS menu-bar template tray icon (alpha + black; OS recolours).

`apple-touch-icon.png` composites the app icon full-bleed over an opaque copy of its own navy badge gradient (`#2E3D65` → `#110222`) so no pixel is transparent — iOS otherwise paints transparent corners black.
