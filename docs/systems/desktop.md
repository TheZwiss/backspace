...
All desktop and web brand assets are generated by `scripts/gen-icons.mjs` from sources in `assets/brand/`. Run via `pnpm gen-icons` after artwork changes; commit the diff. The generator uses `sharp` (SVG → PNG via librsvg; every source is a flat composition with no `<defs>` and no filter), `png-to-ico` (multi-size `.ico`), and `png2icons` (`.icns`). Output is byte-stable for a given lockfile.

**Sources:**
- `app-icon.svg`: a flat lavender (`#7c6cf6`) squircle path plus a flat white glyph path, at a 256 viewBox. Every app-icon output above 32 px renders straight from this vector, at target size, with no raster intermediate or post-render masking.
- `app-icon-small.svg`: the same flat squircle-plus-glyph composition with a bolder, simplified mark. Drives app-icon outputs at 16 and 32 px, where the standard mark's inset strokes read as noise.
...
- `mark-mono-light.svg`: the flat white glyph, for rendering on a lavender ground. Drives the PWA maskable icon's inner mark (composited over a full-bleed flat-lavender canvas, mark at 60% of canvas height) and `packages/web/public/icons/logo-mark.svg` (byte copy), which paints the sidebar's lavender home tile.
...
`mark.svg` (the flat-lavender standalone glyph, for dark grounds) is not read by this script; it feeds `gen-social-preview.mjs` directly.

`apple-touch-icon.png` composites the app icon full-bleed over an opaque copy of its own flat lavender fill (`#7c6cf6`, matching `app-icon.svg`'s squircle) so no pixel is transparent. iOS otherwise paints transparent corners black.
