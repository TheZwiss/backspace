...
- `mark.svg` — the bare gradient glyph. Drives the PWA maskable icon's inner mark (composited over a full-bleed copy of the badge's navy gradient, mark at 60% of canvas height), and `packages/web/public/icons/logo-mark.svg` (byte copy).
- `mark-small.svg` — a bolder small-size variant of the glyph. Drives the web favicons (16/32, transparent, glyph fills the box) and the colour tray icons (Windows `.ico`, Linux PNG).
- `mark-tray.svg` — mono silhouette tuned for 22px. Drives the macOS menu-bar template tray icon (alpha + black; OS recolours). `mark-mono-dark.svg` has no pipeline output today.
