...
- UI components read strings with `useTranslation()` and semantic keys such as
  `common.settings` or `auth.usernameRequired`.
...
3. Run `pnpm --filter @backspace/web typecheck` and
