...
- **`<!-- BUILD:SUMMARY -->`** — a paragraph of headline measurements: the latest measured value of each counter with the date it was measured, the archive totals for views and clones with each one's peak day, app downloads, the leading referrer and path, how many releases shipped and which is newest, and the archive's coverage and resolution. Rendered by `renderSummaryHtml` in `summary.ts`.
  ...
  **The traffic figures here and in the lead row are the same number, and a divergence is a bug.** An earlier revision of this section claimed the opposite, on the belief that the row's views figure was a rolling 30-day window; it is not. `flowCard` sums every measured value in the series, and the 30 days govern only the delta chip beside the figure. So `renderSummaryHtml` is a **second implementation of one figure, in a second language** — the only figure on the page computed anywhere but `INSIGHTS_FIGURES`, which exists precisely so the row and the group heads cannot disagree.

  That equality is asserted rather than trusted: `insights-check.mjs` reads the views and clones totals out of the built paragraph, reads the same two figures off the rendered page, and prints `MATCH` or `DIVERGED` for each. Nobody sees both printings at once — a reader without JavaScript sees only the paragraph, a reader with it sees only the row — so a divergence is invisible on the page by construction and can only be caught there.
...
**The lead figure row is the page's table of contents.** Four figures, in page order: page views linking to `#reach`, stars to `#following`, app downloads to `#adoption` and releases shipped to `#delivery`. Each is an `<a>` rather than a div, so it is keyboard reachable and announced as a link, and each is printed a second time as its group's own headline above that group's hero. Both printings come out of one `INSIGHTS_FIGURES.figure` call per figure per render, which is why that surface exists: the row and a group head printing different numbers for the same series is a failure nothing else on the page would catch. Their window is a fixed 30 days ending on the archive's newest measured day rather than the selected range, so the four stay comparable to each other whatever the control shows, and the row sits **above** the range control for that reason. Nothing is curated: a figure is printed at whatever value it holds and at full precision, and a refused trend never suppresses the figure beside it.
