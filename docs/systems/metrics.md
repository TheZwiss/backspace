...
- **`<!-- BUILD:SUMMARY -->`** — a paragraph of headline measurements: the latest measured value of each counter with the date it was measured, the archive totals for views and clones with each one's peak day, app downloads, the leading referrer and path, how many releases shipped and which is newest, and the archive's coverage and resolution. Rendered by `renderSummaryHtml` in `summary.ts`.
  ...
  **The traffic figures here are archive totals, and the lead row's are a rolling 30-day window. That is deliberate.** A windowed figure baked into a served page goes stale the moment the window moves and nothing on the page can tell; a total across measured days stays true of the archive it was built from. The two are labelled as different figures, and finding them unequal is finding the design rather than a bug. It also avoids a second implementation of the page's window arithmetic in TypeScript, which could disagree with the page's own — the disagreement `INSIGHTS_FIGURES` exists to make impossible for the row and the group heads.
...
`scripts/metrics`'s `test` script is `tsc --noEmit && vitest run` — it runs through the existing root `pnpm -r test` step with no `ci.yml` change required, and covers both types and behavior in one script. All tests are fixture-driven and touch no network; filesystem tests use a per-test `mkdtempSync` directory, cleaned up in `afterEach`. As of this writing there are **405 tests across 15 files**, all passing:
