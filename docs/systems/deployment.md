...
**What proxy-awareness costs, and the one thing the operator owns.** `trustProxy: true` trusts every hop, so the address the app calls the client (`request.ip`) is the **left-most** value in `X-Forwarded-For`, whoever wrote it. Every rate limit in the app keys on that address and has nothing else to key on (see [api.md](api.md), "Rate limiting"), so a limit is only as good as the header:

- **Mode 1, all-in-one (the default).** The bundled Caddy overwrites `X-Forwarded-For` and ignores what the client sent, so the address is real and the limits work. Nothing to do.
- **Mode 2, proxy.** It depends on the proxy's configuration, and the nginx snippet this installer prints uses `$proxy_add_x_forwarded_for`, which **appends** to the client's value rather than replacing it. On such a deployment a client can send its own `X-Forwarded-For` and pick the address every rate limit buckets it under, including the login and registration limits. If that matters for your instance, set `proxy_set_header X-Forwarded-For $remote_addr;` in your own config (and the equivalent elsewhere: Apache `RemoteIPHeader` with `RemoteIPTrustedProxy`, HAProxy `http-request set-header X-Forwarded-For %[src]`). Do not do this if a CDN sits in front of your proxy, because then your proxy's peer is the CDN and the real client address is exactly what the header carries.
- **Mode 3, tunnel.** Ask the provider what it does with the header; Cloudflare overwrites it, others vary.

An evaded rate limit is not an authentication bypass. Every route still authenticates and authorizes; what is lost is the brake on brute force and flooding. Turning `trustProxy` off is not the answer either: the app would then see the proxy's address for every request and put every client in one bucket, which is worse. The narrow fix is a hop count (`trustProxy: 1`), which breaks two-proxy and CDN deployments, so it is not a default this project changes silently. See [web-security.md](web-security.md) section 9.
