...
**The trusted hop count (`TRUSTED_PROXY_HOPS`).** `trustProxy` is not `true`; it is a count of the proxies in front of the app that are trusted to have written `X-Forwarded-For`. It is set in `.env` like every other deployment fact, defaults to **1**, and is read and validated in `packages/server/src/config.ts`; a value that is not a non-negative integer stops the server at boot rather than quietly becoming 1, and so does anything above 4 (a CDN in front of your own proxy in front of a tunnel is 3, so a larger number is a typo, and a typo like 11 would trust the whole forwarded chain again; the boot message names the cap and where to change it). `docker-compose.yml` passes `.env` to the container wholesale (`env_file`), so setting it there is all it takes. `request.ip` is therefore the address the nearest proxy appended, not the left-most thing in the header, so a client cannot choose the address its rate limits are counted under. Every rate limit in the app keys on that address (see [api.md](api.md), "Rate limiting"), as does the unauthenticated federation peer-accept limiter, and it is what the request log records.
...
- **A CDN in front of your own reverse proxy** is two hops. Left at 1, the app sees the CDN's address and everyone behind it shares a rate-limit bucket. Set `TRUSTED_PROXY_HOPS=2`.
