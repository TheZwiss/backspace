...
**Server proxy-awareness.** The server sets Fastify `trustProxy` to a count of trusted hops (1, see below), so it trusts `X-Forwarded-*` from the fronting proxy/tunnel but not from anything beyond it. `getOurOrigin()` returns `https://${DOMAIN}` (federation/public identity) whenever `DOMAIN` is set and `PUBLIC_ORIGIN` is unset — correct in all three modes, since the public URL is `https://DOMAIN` regardless of which layer terminates TLS. No `PUBLIC_ORIGIN` is needed for a normal proxy/tunnel deployment.

**The trusted hop count.** `trustProxy` is not `true`; it is a count of the proxies in front of the app that are trusted to have written `X-Forwarded-For`, and it ships at **1** (`packages/server/src/utils/trustedProxy.ts`). `request.ip` is therefore the address the nearest proxy appended, not the left-most thing in the header, so a client cannot choose the address its rate limits are counted under. Every rate limit in the app keys on that address (see [api.md](api.md), "Rate limiting"), as does the unauthenticated federation peer-accept limiter, and it is what the request log records.
...
- **A CDN in front of your own reverse proxy** is two hops. Left at 1, the app sees the CDN's address and everyone behind it shares a rate-limit bucket. Raise the constant to 2.
- **No proxy at all**, the app exposed directly on its port, is zero hops. Left at 1, the app believes an `X-Forwarded-For` header a client sends, because from inside the process one proxy and one lying client look the same. Set the constant to 0, which ignores the header and uses the socket address.
