...
```typescript
{
  ...
  directoryAvailable: boolean;  // config.directory.endpoint !== '' AND instanceSettings.directoryBrowseEnabled; the Explore page gates its Outer Space section on it
...
- Global browsing switch, directly under the ladder in the same section, separated by a rule and outside the radiogroup: "Show global spaces in Explore" with "People here see spaces from other instances in Outer Space. Their browsers load those spaces' icons and banners from the instances that own them." It writes `directoryBrowseEnabled`, the incoming axis, and is a draft field saved by the same save bar as the rungs, not an immediate write. On an instance with no `DIRECTORY_ENDPOINT` the switch renders **off** and disabled, with "This instance is not configured to reach a directory, so there is nothing to show." beneath it: off is the effective state whatever the column says, and the column is deliberately not written to match, so browsing resumes at the admin's last choice if an endpoint is ever configured. The panel works the endpoint out from `GET /api/instance/info` and nothing was added to the API for it. `directoryAvailable` is the endpoint and the setting together, so the panel reduces that answer and the saved setting to the single fact "has an endpoint" (available proves one; unavailable with browsing on proves none; unavailable with browsing off proves nothing and leaves the previous answer standing) and stores the reduction, not the raw flag. Storing the raw flag instead splits the pair: a save writes the setting at once and the flag is a round trip behind it, so the panel would read one value from before the change and one from after for the length of the post-save re-read. The two are not read at the same instant and cannot be, since the server reads its half when it handles the request and the client reads its half when the answer arrives; what makes the pairing sound is that the setting moves only through this panel's own save, which bumps `saveEpoch` before it writes (refusing any answer whose request predates it, which the effect's own cleanup does not cover: an answer landing after the store write but before React commits the re-render would otherwise slip through) and `directoryProbe` after it (asking again). The probe is gated on the settings being loaded, because the reduction needs the browse setting and `InstancePanel` renders this panel in the same commit as its own `fetchInstanceSettings()` effect: a child's effects run before its parent's, so an ungated request would go out against an empty store, learn nothing, and leave the row unknown for the life of the modal. A failed re-read changes nothing. Strings under `admin:general.browse.*`. See [directory.md](directory.md) §10.
